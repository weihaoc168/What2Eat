...
def main():
    ...
    top_cuisines = [c for c, w in pref if c not in ("咖啡", "奶茶", "果汁", "甜品")][:12]
    ...
    for round_no in range(4):
        ...
        if len(cur) >= target:
            break
        print(f"— 第 {round_no + 1} 轮（已有 {len(cur)}/{target}）—", flush=True)
        hints = [
            "",
            "这一轮把范围扩大到周边城市：Friendswood、League City、Sugar Land、Bellaire 唐人街、Katy Asian Town 一带。",
            "这一轮按具体品类搜索：早午餐 brunch、火锅、日式拉面、韩式烤肉、越南粉、印度咖喱、湘菜川菜。",
            "这一轮找 2025-2026 年新开业或本地人推崇的 hidden gems（本地论坛/Reddit/美食榜单）。",
        ]
        added = run_round(client, area, top_cuisines, existing + [r["name"] for r in cur], hints[round_no % len(hints)])
        print(f"  本轮新增 {added}", flush=True)
    final = json.load(open(OUT, encoding="utf-8")) if os.path.exists(OUT) else []
    print(f"共 {len(final)} 家「猜你喜欢」")
    return


def run_round(client, area, top_cuisines, existing, hint=""):
    response = client.messages.create(
        ...
        messages=[{"role": "user", "content": (
            ...
            f"对每个菜系分别执行一次网络搜索，找出当地评分最高的餐厅（Google 评分 4.2 以上即可收录），每个菜系尽量给足 3 家，"
            ...
            "rating 优先用搜索到的真实 Google 评分；本地美食榜单/评论文章推荐的真实餐厅即使查不到精确评分也可以收录（rating 设为 null）。绝不虚构不存在的餐厅。"
        )}],
    )
