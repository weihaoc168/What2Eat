...
**全家共享清单**：在部署主机上跑 `python scripts/sync_server.py`（默认 :8125，同时托管应用本身），家人打开该地址即共享同一份周计划和采购清单 —— 一起逛超市时谁划掉一项，其他人手机上实时同步。从静态托管打开时此功能自动隐藏。

## 下馆子（周末外食推荐）

第四个标签页「下馆子」管吃外面：每日推荐一家（按口味偏好加权，可换一家）、想去/去过清单带真实门面照片和健康标签（高蛋白/高碳水/高胆固醇等，AI 分类），支持健康筛选（忌高胆固醇/忌高饱和脂肪/忌反式脂肪），每家直达 Google 地图。`scripts/discover_restaurants.py` 还能按你的口味联网搜索同城同类高分餐厅，生成「猜你喜欢」推荐（需 API Key）。
